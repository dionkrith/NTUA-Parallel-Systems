...
int main(int argc, char ** argv) {
    ...
    if (rank==0)
        ...
        /*Datatype that refers to a column of a local block*/
        MPI_Datatype column;
    ...
    int i_min,i_max,j_min,j_max;
        ...
        */
        ...
        //ranges for internal process
        i_min = 1;
        i_max = local[0] +1;
        j_min = 1;
        j_max = local[1] + 1;

        //not have a north neighboor (boundary process but not padded global array)
        if (north == MPI_PROC_NULL){
                i_min++;
        }

        //not have a south neighboor (boundary process and possibility for padded gloabl array)
        if (south == MPI_PROC_NULL){
                i_max = i_max - (global_padded[0]-global[0]) -1;
        }

        //not have a west neighboor (boundary process but not padded global array)
        if (west == MPI_PROC_NULL){
                j_min++;
        }

        //not have a east neighboor (boundary process and possibility for padded global array)
    ...
    for (t=0;t<256;t++) {
                ...
                if (north != MPI_PROC_NULL){
                        MPI_Isend(&u_previous[1][1], local[1] , MPI_DOUBLE , north , 0 , MPI_COMM_WORLD , &req_bef[req_num_bef]);
                        req_num_bef++;
                        MPI_Irecv(&u_current[0][1], local[1] , MPI_DOUBLE , north , 0 , MPI_COMM_WORLD , &req_bef[req_num_bef]);
                        req_num_bef++;
                }

                /*Send previous data to west and receive current data from west*/
                if (west != MPI_PROC_NULL){
                        MPI_Isend(&u_previous[1][1], 1 , column , west , 0 , MPI_COMM_WORLD , &req_bef[req_num_bef]);
                        req_num_bef++;
                        MPI_Irecv(&u_current[1][0], 1 , column , west , 0 , MPI_COMM_WORLD , &req_bef[req_num_bef]);
                        req_num_bef++;
                }

                /*Receive previous data from south*/
                if (south != MPI_PROC_NULL){
                        MPI_Irecv(&u_previous[local[0]+1][1] , local[1] , MPI_DOUBLE , south , 0 , MPI_COMM_WORLD , &req_bef[req_num_bef]);
                        req_num_bef++;
                }

                /*Receive previous data from east*/
                if (east !=MPI_PROC_NULL){
                        MPI_Irecv(&u_previous[1][local[1]+1] , 1 , column , east , 0 , MPI_COMM_WORLD , &req_bef[req_num_bef]);
                        req_num_bef++;
                }
                ...
                for (i = i_min ; i<i_max ; i++){
                        for (j = j_min; j<j_max; j++){
                                u_current[i][j] = u_previous[i][j] + (omega/4.0)*(u_current[i-1][j] + u_current[i][j-1] + u_previous[i][j+1] + u_previous[i+1][j] - 4*u_previous[i][j]);
                        }
                }
                ...
                if (south != MPI_PROC_NULL){
                        MPI_Isend(&u_current[local[0]][1] , local[1] , MPI_DOUBLE ,south , 0 , MPI_COMM_WORLD , &req_after[req_num_after]);
                        req_num_after++;
                }

                /*Send current data to east*/
                if (east != MPI_PROC_NULL){
                        MPI_Isend(&u_current[1][local[1]], 1 , column , east , 0 , MPI_COMM_WORLD , &req_after[req_num_after]);
                        req_num_after++;
                }
