...
int main(int argc, char ** argv) {
    ...
    for (t=0;t<256;t++) {
        ...
        if (north != MPI_PROC_NULL){
                ...
                req_len++;
                MPI_Irecv(&(u_previous[0][1]),local[1],MPI_DOUBLE,north,0,MPI_COMM_WORLD,&req[req_len]);
                req_len++;
        }
        ...
        if (south != MPI_PROC_NULL){
                ...
                req_len++;
                MPI_Irecv(&(u_previous[local[0]+1][1]),local[1],MPI_DOUBLE,south,0,MPI_COMM_WORLD,&req[req_len]);
                req_len++;
        }
        ...
        if (west != MPI_PROC_NULL){
                ...
                req_len++;
                MPI_Irecv(&(u_previous[1][0]),1,column,west,0,MPI_COMM_WORLD,&req[req_len]);
                req_len++;
        }
        ...
        for (j=j_min;j<j_max;j++){
                u_current[i_min][j] = (u_previous[i_min-1][j] + u_previous[i_min+1][j] + u_previous[i_min][j-1] + u_previous[i_min][j+1])/4.0;
                u_current[i_max-1][j] = (u_previous[i_max-1-1][j] + u_previous[i_max-1+1][j] + u_previous[i_max-1][j-1] + u_previous[i_max-1][j+1])/4.0;
         }

         for (i=i_min;i<i_max;i++){
                u_current[i][j_min] = (u_previous[i-1][j_min] + u_previous[i+1][j_min] + u_previous[i][j_min-1] + u_previous[i][j_min+1])/4.0;
                u_current[i][j_max-1] = (u_previous[i-1][j_max-1] + u_previous[i+1][j_max-1] + u_previous[i][j_max-1-1] + u_previous[i][j_max-1+1])/4.0;
         }
        gettimeofday(&tcf,NULL);
        tcomp = tcomp + (tcf.tv_sec - tcs.tv_sec) + (tcf.tv_usec-tcs.tv_usec)*0.000001;
